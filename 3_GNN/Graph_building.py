...
def build_graph(df, cell_features=None, method='radius', k=10, radius=60, min_degree=3):
    ...
    # set up connection for those low connectivity nodes
    ...
    pos = torch.tensor(coords_original[perm], dtype=torch.float)  # original coordinate
        ...
        if file.endswith(".txt"):
            ...
            try:
                ...
                if not txt_path.exists():
                    logging.warning(f"[Skip] TXT：{txt_path}")
                    continue
                ...
                logging.info(f"[Done] {worm_id} → {save_path}")
                processed += 1

            except Exception as e:
                logging.error(f"[ERROR] {worm_id} goes wrong：{e}", exc_info=True)
                failed.append(worm_id)

    logging.info(f"\nprocessed {processed} graph")
    if failed:
        logging.warning(f"⚠️ there are {len(failed)} fail：{failed}")
...
if __name__ == "__main__":
    ...
    # report
    print(f"Max edge: {max(all_degrees)}")
    print(f"Min edge: {min(all_degrees)}")
    print(f"Avg. edge: {sum(all_degrees) / len(all_degrees):.2f}")
    print(f"which cell has the most edge: {set(max_degree_names)}")
    print(f"which cell has the fewest edge: {set(min_degree_names)}")
